...
class InvoiceService {
  ...
  private handleError(error: any, action: string): void {
    if (error.response) {
      ...
      switch (status) {
        case 400: console.error(`❌ Bad request when trying to ${action}:`, message); break;
        case 401: console.error(`❌ Unauthorized - please login again`); break;
        case 403: console.error(`❌ Forbidden - you don't have permission to ${action}`); break;
        case 404: console.error(`❌ Invoice not found`); break;
        case 500: console.error(`❌ Server error when trying to ${action}:`, message); break;
        default:  console.error(`❌ Error ${status} when trying to ${action}:`, message);
      }
